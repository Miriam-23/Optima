...
class ReporteView(APIView):
    ...
    def _reporte_miembro(self, usuario, hoy):
        ...
        completadas = mis_tareas.filter(estado__nombre='Completada').count()
        en_progreso = mis_tareas.filter(estado__nombre='En progreso').count()
        pendientes = mis_tareas.exclude(
            estado__nombre__in=['Completada', 'En progreso']
        ).count()
        vencidas = mis_tareas.filter(
            fecha_limite__lt=hoy
        ).exclude(estado__nombre='Completada')
        ...
        for proyecto in proyectos:
            ...
            carga_por_proyecto.append({
                ...
                'completadas': tareas_proyecto.filter(estado__nombre='Completada').count(),
            })

        # Tareas críticas — vencidas o de alta prioridad sin completar
        tareas_criticas = list(
            mis_tareas.exclude(estado__nombre='Completada')
    ...
    def _reporte_pm(self, usuario, hoy):
        ...
        completadas = todas_tareas.filter(estado__nombre='Completada').count()
        ...
        vencidas = todas_tareas.filter(
            fecha_limite__lt=hoy
        ).exclude(estado__nombre='Completada').count()
        ...
        for proyecto in proyectos:
            ...
            completadas_p = tareas_p.filter(estado__nombre='Completada').count()
        ...
        fuegos = list(
            todas_tareas.exclude(estado__nombre='Completada')
            .filter(prioridad='Alta')
            .order_by('fecha_limite')
            .values('id', 'titulo', 'fecha_limite', 'prioridad', 'proyecto__nombre')[:5]
        )
